...
require("knitr")
opts_knit$set(root.dir = "D:/Work/Sims - DROPS Cali")
...
Fatality <- function(filename)
{
  ...
  print('NO FATALITY !')
  play(readMP3(filename))
} 

Fatality('C:/Users/lacube/Music/Fatality.mp3')
...
levels(Measured_Data_GN$Title) <- c('Martonvasar_2013','Gaillac_2012','Gaillac_2013','Graneros_2013',
                                 ...
                                 'Karlsruhe_2013','Murony_2013','Karcag_2012',
...
write.table(Measured_Data_GN,
            paste0(PATHS[3],'/Measured_Data_GN.csv'),
            sep=";",
            row.names=F)
...
ggplot(Measured_Data_GN,aes(x=reorder(Title,M.GrainNo,FUN=mean,na.rm=T),y=M.GrainNo))+
  ...
  labs(x='Environment',y='Grain Number (grain.m²)')+
  coord_flip()
...
PotKernelWt_2 <- subset(merge(PotKernelWt,MeasuredYieldwithDensity),
                        select=c('Genotype','KernelWt','Title'))
colnames(PotKernelWt_2) <- c('Hybrid_ID','potKernelWt','potKernelWt_At')
GNmax_2 <- subset(merge(GNmax,MeasuredYieldwithDensity),
                  select=c('Genotype','KNOplant','Title'))
...
RespTnight <- subset(M.Tnight,select=c('Hybrid_ID','resp.tnight.seq','GN15cor','resp.ri.sequ'))
...
SensiRAD <- subset(SensiRAD,select=c('Hybrid_ID','SensiRAD'))
...
```{r Params Distributions, eval=FALSE, fig.height=4, fig.width=6, message=FALSE, include=FALSE}


PhylloAllManips <- read.table(paste0(PATHS[3],'/Phyllo_Parameters_AllManips.csv'), 
                       sep=",",                                           
                       header=TRUE,dec=".",
                       row.names=NULL,
                       as.is=FALSE, 
...
ToPlot <- merge(PhylloAllManips,OutPhenoPanel,by='Hybrid_ID')

# Compare ZA16 to all experiments :
ToPlotPhyllo <- melt(ToPlot, measure.vars=c('Phyllochron','Phyllo'))

ggplot(ToPlotPhyllo,aes(x=value,color=variable)) +
...
MyParams <- subset(Parameters,select=colnames(Parameters)[c(2:7,9,17,8,10,11,18)])
...
MatrixCorr <- cor(MyParams[c(2:8)],use="complete.obs")
chart.Correlation(MyParams[c(2:3,5,7,8,10:12)], histogram=T, pch=21)
...
```{r Prepare file for GWAS, echo=FALSE, fig.height=3, fig.width=3, message=FALSE, warning=FALSE}

ggplot(data=MyParams,aes(x=a,y=c))+
  geom_point(color='black')+
  geom_smooth(method='lm',color='blue',se=F)+
  scale_x_continuous(limits=c(2.5,6.2))+
  scale_y_continuous(limits=c(3,10))+
  theme_bw()+
  labs(x=expression(paste('a (mm.°Cd'^'-1',')')),
       y=expression(paste('b (mm.°Cd'^'-1','.kPa)'^'-1')))
  


ggplot(data=MyParams,aes(x=Phyllochron,y=First_LiguloChrone))+
  geom_point(color='black')+
  geom_smooth(method='lm',color='blue',se=F)+
  scale_x_continuous(limits=c(0.0170,0.0285))+
  scale_y_continuous(limits=c(0.012,0.018))+
  theme_bw()+
  labs(x=expression(paste('leaf.°Cd'^'-1')),
       y=expression(paste('ligule.°Cd'^'-1')))
```
...
for (i in Stopped_At:length(unique(Params_WithoutNAs$Hybrid_ID)))
{
  ...
  if (nrow(ParamsToUse)>0)
  {
    ...
    tryCatch(  system(command,show.output.on.console=F)
               , 
               error= function(){}
    )
...
FileName<-paste('Global_Output_LastCali3.RData')

# Load Rdata!!
load(paste(PATHS[4],FileName,sep='/'))

# Get Harvest data :
HarvestData <- Get_Harvest_Data(Global_Output_GenoPotKW_Cali, # My Data out of APSIM
                                list(Global_Output_GenoPotKW_Cali$Title,
                                     Global_Output_GenoPotKW_Cali$Genotype)) # On which factors do we work ?
rownames(HarvestData) <- NULL


head(Global_Output_GenoPotKW_Cali)
```
...
Data <- subset(distinct(HarvestData),select=c('DaysAfterSowing','yield','Biomass','GrainNo','LAI','Title','Genotype'))
...
ToPlot <- subset(SimulatedAndMeasured_GY,select=c('Genotype','Title','M.Yield','yield'))
colnames(ToPlot) <- c('Hybrid','Site','Measured','Simulated')

unique(SimulatedAndMeasured_GY)

ggplot(ToPlot,aes(x=Measured,y=Simulated/1000,color=Site))+
  geom_abline(slope=1,intercept=0,color='black')+
  geom_point()+
  geom_point(data=subset(ToPlot,Hybrid=='B73_H'),aes(x=Measured,y=Simulated/1000),color='red',size=4)+
...
ToPlot <- subset(distinct(SimulatedAndMeasured_GN),select=c('Genotype','Title','M.GrainNo','GrainNo'))
colnames(ToPlot) <- c('Hybrid','Site','Measured','Simulated')
